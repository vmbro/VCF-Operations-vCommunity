        ...
        for vibName in esxiVIBDrivers:
            for package in softwarePackages:
                if vibName == package.name:
                    #host_obj.with_property(f"vCommunity|Configuration|Packages|{package.name}|Name", package.name)
                    #host_obj.with_property(f"vCommunity|Configuration|Packages|{package.name}|Version", package.version)
                    ...
                    #host_obj.with_property(f"vCommunity|Configuration|Packages|{package.name}|Summary", package.summary)
                    #host_obj.with_property(f"vCommunity|Configuration|Packages|{package.name}|Type", package.type)
                    #host_obj.with_property(f"vCommunity|Configuration|Packages|{package.name}|Vendor", package.vendor)

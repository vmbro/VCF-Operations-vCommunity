...
5. ESXi VIB Driver Config File: esxi_packages.  This can be adjusted to include additional Packages via Infrastructure Operations - Configurations - Management Pack Configuration - System Defined - esxi_packages.xml.  These are the System - Packages as seen on ESXi Hosts in vCenter via the Configure tab.
...
7. VM Configuration Config File: vm_options.  This can be adjusted to include additional Configurations via Infrastructure Operations - Configurations - Management Pack Configuration - System Defined - vm_options.xml.  These are the VM Options as seen on VMs in vCenter via Edit Settings.  For Configuration names go to the vCenter MOB for the VM in question and the config option.
